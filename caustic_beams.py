...
N_ROOTS = 25
# in case of pearcey beam: should the results of method of steepest descent
# and integration method be merged together? This reduces the number of weights
# needed. Otherwise, only integration method is used.
USE_MASK = True
...
def _get_coeff(pos, neg=False):
    ...
    coeff = [-D]
    for i in xrange(ORDER - 2):
        coeff = [(fac2 ** (i+1)) * pos[i] * np.sin(fac * (i+1) * np.pi / (2*ORDER))] + coeff
    coeff = [1, 0] + coeff
    return coeff
...
f = interpolate.interp2d(range_coarse[0], range_coarse[1], roots_coarse, kind='linear')
f2 = interpolate.interp2d(range_coarse[0], range_coarse[1], roots_coarse_neg, kind='linear')
roots_fine = f(range_fine[1], range_fine[0])
roots_fine_neg = f2(range_fine[1], range_fine[0])
...
def _plot(mat, vmax, i):
    plt.subplot(2, 1, i)
    plt.pcolormesh(axes[0], axes[1], mat, vmax=vmax)
    plt.colorbar()
